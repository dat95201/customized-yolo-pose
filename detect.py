...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            fps = str(round(1/(t2 - t1), 2)) + ' FPS (' + str(round(sum(mTime)/len(mTime),2)) + ')'
            cv2.putText(im0, fps, (8, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 255, 0), 2, cv2.LINE_AA)
            print(f'{s}Done. ({t2 - t1:.2f}s)')
    ...
    print(f'Average inference time: {sum(mTime)/len(mTime):.3f} FPS')
